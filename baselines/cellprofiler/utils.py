...
def _is_lfs_pointer(filepath):
    ...
    try:
        with open(filepath, "rb") as f:
            head = f.read(80).decode("utf-8", errors="ignore")
        return "version https://git-lfs.github.com" in head
    except Exception:
        return False


def load_precomputed_benchmark_results(output_dir="output"):
    """Load precomputed benchmark results when profile files are Git LFS pointers.

    Use this when profiles are not downloaded (LFS pointers). Returns the same
    dataframes produced by the benchmark notebook.
    """
    base = os.path.join(os.path.dirname(__file__), output_dir)
    replicability_map_df = pd.read_csv(os.path.join(base, "cellprofiler_replicability_map.csv"))
    replicability_fr_df = pd.read_csv(os.path.join(base, "cellprofiler_replicability_fr.csv"))
    matching_map_df = pd.read_csv(os.path.join(base, "cellprofiler_matching_map.csv"))
    matching_fr_df = pd.read_csv(os.path.join(base, "cellprofiler_matching_fr.csv"))
    gene_compound_matching_map_df = pd.read_csv(
        os.path.join(base, "cellprofiler_gene_compound_matching_map.csv")
    )
    gene_compound_matching_fr_df = pd.read_csv(
        os.path.join(base, "cellprofiler_gene_compound_matching_fr.csv")
    )
    return (
...
def load_data(exp, plate, filetype, profiles_dir=None):
    """load all data from a single experiment into a single dataframe"""
    if profiles_dir is None:
        profiles_dir = os.path.join(os.path.dirname(__file__), "..", "profiles")
    path = os.path.join(profiles_dir, f"{exp}", f"{plate}", f"*_{filetype}")
    files = glob.glob(path)
    if not files:
        raise FileNotFoundError(f"No files found for {path}")
    dfs = []
    for fpath in files:
        # Check for LFS pointer before attempting read (avoids BadGzipFile traceback)
        if _is_lfs_pointer(fpath):
            raise FileNotFoundError(
                "Profile files are Git LFS pointers (real data not downloaded). "
                "The upstream repo's LFS quota is often exceeded. Either run "
                "'git lfs pull' (if quota is fixed) or generate profiles by "
                "downloading backend from S3 and running the pycytominer pipeline "
                "(see README: 'If Git LFS quota is exceeded'). "
                "To use precomputed results without profiles, call "
                "utils.load_precomputed_benchmark_results() and set USE_PRECOMPUTED=True."
            )
        try:
