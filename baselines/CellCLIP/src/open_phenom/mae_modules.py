...
class MAEEncoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        if channel_agnostic:
            self.vit_backbone = channel_agnostic_vit(vit_backbone, max_in_chans=max_in_chans)
        else:
    ...
    def forward_masked(
        ...
        x: torch.Tensor,
        mask_ratio: float,
        constant_noise: torch.Tensor | None = None,
...
class MAEDecoder(nn.Module):
    ...
    def forward_masked(self, x: torch.Tensor, ind_restore: torch.Tensor) -> torch.Tensor:
        mask_tokens = self.mask_token.repeat(x.shape[0], ind_restore.shape[1] + 1 - x.shape[1], 1)
...
class CAMAEDecoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x + self.pos_embeddings + modality_tokens_concat  # add pos and tiled modality tokens
        x_ = x[:, 1:, :]  # no class token
        for m, decoder in enumerate(self.decoders):  # iterate through modalities and decoders
