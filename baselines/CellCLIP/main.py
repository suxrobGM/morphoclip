    ...
    while steps < total_steps:
        for _, batch in enumerate(train_dataloader):
            ...
            if args.model_type == "clip_channelvit":
                img_features, text_features, logit_scale = model(images, extra_tokens, treatments)
            else:
                if args.loss_type in ["s2l", "sigclip"]:
                    img_features, text_features, logit_scale, bias = model(images, treatments)
                else:
                ...
                if steps % args.eval_freq == 0 or steps == 1 or steps == total_steps:
                    ...
                    with torch.no_grad():
                        ...
                        if accelerator.is_main_process:
                            ...
                            metrics = get_metrics(all_eval_image_features, all_eval_text_features)
