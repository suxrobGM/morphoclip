...
def percent_score(null_dist, corr_dist, how):
    ...
    if how == "both":
        ...
        below_threshold = corr_dist < perc_5
        return (
            (np.mean(above_threshold.astype(float)) + np.mean(below_threshold.astype(float))) * 100,
            perc_95,
            perc_5,
        )
...
def correlation_between_modalities(
    modality_1_df, modality_2_df, modality_1, modality_2, metadata_common, metadata_perturbation
):
    """
    ...
    list_common_perturbation_groups = list(
        np.intersect1d(list(modality_1_df[metadata_common]), list(modality_2_df[metadata_common]))
    )
...
def null_correlation_between_modalities_list(
    modality_1_df,
    modality_2_df,
    modality_1,
    modality_2,
    ...
    while len(null_modalities) < n_samples:
        ...
        if modality_1 == "compound":
            modality_1_perturbation_list = np.unique(
                modality_1_perturbation_df.Metadata_gene_list.sum()
            )
            if perturbations[1] not in modality_1_perturbation_list:
                overlap = False
        elif modality_1 == "compound":
            modality_2_perturbation_list = np.unique(
                modality_2_perturbation_df.Metadata_gene_list.sum()
            )
            if perturbations[0] not in modality_2_perturbation_list:
                overlap = False
...
def sphere_plate_zca_corr(plate):
    ...
    feature_df = pd.DataFrame(sphered_vals, columns=get_featurecols(plate), index=plate.index)
...
class PrecisionScores:
    ...
    def __init__(
        ...
        self.map1 = self.profile1[
            [self.feature, self.sample_feature, self.control_type_feature]
        ].copy()
    ...
    def process_profiles(self, _profile):
        ...
        _perturbation_id_df = pd.DataFrame(
            {self.sample_feature: [f"sample_{i:0{width}}" for i in range(len(_metadata_df))]}
        )
    ...
    def create_truth_matrix(self):
        ...
        _truth_matrix = _truth_matrix.merge(
            self.map2, left_on="level_0", right_on=self.sample_feature, how="left"
        ).drop([self.sample_feature, 0], axis=1)
        _truth_matrix = _truth_matrix.merge(
            self.map1, left_on="level_1", right_on=self.sample_feature, how="left"
        ).drop([self.sample_feature], axis=1)
        _truth_matrix["value"] = np.where(
            _truth_matrix[f"{self.feature}_x"] == _truth_matrix[f"{self.feature}_y"], 1, 0
        )
        if self.within:
            _truth_matrix["value"] = np.where(
                _truth_matrix["level_0"] == _truth_matrix["level_1"], 0, _truth_matrix["value"]
            )
        _truth_matrix = (
    ...
    def calculate_average_precision_at_k_per_sample(self):
        ...
        if self.challenge_negcon:
            _pk_sample_df = (
                _pk_sample_df.query(f'{self.control_type_feature}!="negcon"')
                .drop(columns=[self.control_type_feature])
                .reset_index(drop=True)
            )
        else:
            _pk_sample_df = _pk_sample_df.drop(columns=[self.control_type_feature]).reset_index(
                drop=True
            )
    ...
    def calculate_average_precision_at_r_per_sample(self):
        ...
        if self.challenge_negcon:
            _pr_sample_df = (
                _pr_sample_df.query(f'{self.control_type_feature}!="negcon"')
                .drop(columns=[self.control_type_feature])
                .reset_index(drop=True)
            )
        else:
            _pr_sample_df = _pr_sample_df.drop(columns=[self.control_type_feature]).reset_index(
                drop=True
            )
    ...
    def process_negcon(self, _corr_df):
        ...
        if self.challenge_negcon:
            ...
            _corr_df["filter"] = np.where(
                _corr_df[f"{self.control_type_feature}_x"] == "negcon", 1, _corr_df["filter"]
            )
            _corr_df["filter"] = np.where(
                _corr_df[f"{self.control_type_feature}_y"] == "negcon", 0, _corr_df["filter"]
            )
        else:
            _corr_df["filter"] = np.where(
                _corr_df[f"{self.control_type_feature}_x"] == "negcon", 0, _corr_df["filter"]
            )
            _corr_df["filter"] = np.where(
                _corr_df[f"{self.control_type_feature}_y"] == "negcon", 0, _corr_df["filter"]
            )
...
def precision_at_r_threshold(precision_df, shuffled_precision_df):
    ...
    for threshold_value in threshold_values:
        ...
        df = df.append(
            {"threshold": threshold_value, "percent_perturbations": percent_perturbations},
            ignore_index=True,
        )
