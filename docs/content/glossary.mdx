...
MorphoCLIP's training loss function. Unlike standard contrastive loss (which treats matches as binary: correct or not), CWCL assigns soft weights based on how similar perturbations are, so the model can learn from partial matches.

### CWA (Cross-Well Alignment)

An optional batch correction applied during training to reduce plate-to-plate variation. Each plate's embeddings are shifted by a precomputed offset (the plate's mean embedding minus the mean of its replicate plates, those sharing the same cell line, modality, and timepoint), refreshed once per epoch. Correcting relative to replicate plates removes technical drift without deleting the condition-level signal the text prompts describe.
