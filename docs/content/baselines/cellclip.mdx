...
- `cpjump_matching_eval.py` from the upstream
  [CellCLIP repo](https://github.com/suinleelab/CellCLIP) (no longer vendored here)
- the `morphoclip benchmark` command (`benchmark_stable.py`)
...
Note: `morphoclip benchmark` does not run this embedding branch; it analyzes profile feature columns directly.
...
- Use `morphoclip benchmark` when you want:
  - a reproducible local benchmark run,
  - robust handling of missing/incomplete data,
  - saved artifacts for comparison and reporting.
- Use the upstream [CellCLIP](https://github.com/suinleelab/CellCLIP)
  `cpjump_matching_eval.py` when you specifically need:
