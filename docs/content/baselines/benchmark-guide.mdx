...
- These are produced from per-site feature tensors with [`scripts/cellclip/export_cellclip_profiles.py`](../../scripts/cellclip/export_cellclip_profiles.py)
...
```bash
uv run python scripts/benchmark/benchmark_stable.py \
...
```bash
uv run python scripts/cellclip/export_cellclip_profiles.py \
...
```bash
uv run python scripts/benchmark/compare_benchmark.py \
