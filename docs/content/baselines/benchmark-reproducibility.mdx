...
4. Stage the benchmark metadata into `output/benchmark/input/`. Everything needed
   ships in this repo under `data/reference/cpjump1/`, including a vendored
   `experiment-metadata.tsv` (originally produced by the upstream
   `0.create-experiment-metadata.ipynb` in the Chandrasekaran 2024 repo; rerun
   that notebook if you need to regenerate it, a copy in
   `output/benchmark/input/` takes precedence over the vendored one):
...
cp data/reference/cpjump1/experiment-metadata.tsv output/benchmark/input/
```
