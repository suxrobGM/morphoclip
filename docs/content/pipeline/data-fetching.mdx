...
Images are fetched from the Cell Painting Gallery S3 bucket using a unified downloader (`morphoclip data fetch`, or `uv run poe fetch-dataset`) that supports both **AWS CLI** and **rclone**. Dataset configuration lives in `configs/dataset.yml`.
