...
<Callout type="info" emoji="💡">
  **In Plain English:** Before training, raw microscopy images are converted into compact
  numerical representations (features). This pipeline runs each image through a pre-trained vision
  model (DINOv3) that summarizes it as a list of ~1000 numbers. The features are saved to
