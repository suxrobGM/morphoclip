...
```bash
uv run morphoclip features extract                      # all plates
uv run morphoclip features extract --plate BR00116991   # single plate
uv run morphoclip features extract --verify-only        # check completeness
uv run morphoclip features extract --no-tensors         # skip tensor saving (saves disk space, slightly faster)
```
...
```text
data/
  metadata/                    # downloaded via uv run morphoclip data fetch --metadata
