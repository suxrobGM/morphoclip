...
```bash
uv run morphoclip data fetch --metadata   # downloads platemaps + external_metadata
uv run morphoclip text precompute --metadata-dir data/metadata/external_metadata
uv run morphoclip text precompute --metadata-dir data/metadata/external_metadata --limit 20  # first 20 only
```

## Metadata sources

- **External TSVs** (recommended): `data/metadata/external_metadata/` holds `JUMP-Target-1_compound_metadata_targets.tsv`, `JUMP-Target-1_crispr_metadata.tsv`, `JUMP-Target-1_orf_metadata.tsv`. Get them via `uv run morphoclip data fetch --metadata`.
...
- `--limit N`: process the first N perturbations only
