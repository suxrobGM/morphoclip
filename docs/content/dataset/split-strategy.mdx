...
These are implemented in `src/morphoclip/splits/` and are the only split names that should be used in configs, scripts, and analysis.
...
- benchmark experiment metadata:
  ...
  - fallback: `data/reference/cpjump1/experiment-metadata.tsv` (vendored, always present)
- official CPJUMP1 split metadata:
  - `data/reference/cpjump1/cpjump1_metadata.csv`
