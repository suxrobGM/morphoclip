...
1. `scripts/data/label_generator.py` builds `labels.csv` from image filenames.
2. `morphoclip split` splits `labels.csv` into train/test label files.

## Step 1: Label generation (`scripts/data/label_generator.py`)
...
```bash
uv run python scripts/data/label_generator.py \
