...
A core problem in drug discovery is finding a compound's mechanism of action (MoA): which protein or biological pathway the drug targets. The Cell Painting assay images stained cells at high throughput, but current analysis (CellProfiler handcrafted features plus cosine similarity) detects only 5-25% of expected compound-gene matches (Chandrasekaran et al., 2024). This project asks whether vision-language models can learn richer representations that raise this match rate.
...
**Summary:** Created the CPJUMP1 benchmark dataset, curated so that each perturbed gene's protein product is a known target of at least two chemical compounds. That makes it possible to test whether chemical and genetic perturbations of the same protein produce similar cell morphologies.
...
**Relevance to MorphoCLIP:** This dataset is the ground-truth benchmark. Our model is compared directly against these CellProfiler baselines.
...
**Approach:** First CLIP-style contrastive learning framework for Cell Painting. Trains two encoders jointly (a ResNet for microscopy images, an MLP for Morgan molecular fingerprints) to embed matched image-molecule pairs close together in a shared latent space.
...
- Uses ResNet without cross-channel reasoning; treating the 5 Cell Painting channels like RGB loses channel-specific biological information
...
1. Use a pre-trained phenomics model (Phenom-1) as the image encoder rather than training from scratch, which speeds up training and improves performance
2. Average embeddings across replicates of the same perturbation to reduce noise
3. Encode concentration implicitly and explicitly, treating different drug concentrations as distinct classes
...
**Relevance to MorphoCLIP:** Confirms that pre-trained image encoders and careful loss design matter. Our approach should use pre-trained embeddings rather than train vision models from scratch.
...
- **Text-based perturbation encoding:**
  Prompt template: "A Cell Painting image of [cell_type] cells treated with [drug_name], SMILES: [SMILES_string]". For CRISPR: "...with CRISPR knockout of gene [GENE]". Uses a pre-trained language model (e.g., PubMedBERT) as the text encoder. The SMILES string is the most critical component: removing it causes the largest performance drop.
- **CrossChannelFormer:** A transformer that processes each of the 5 Cell Painting channels separately, then uses cross-attention over inter-channel relationships. Captures channel-specific biology (unlike CLOOME's channel stacking).
- **Attention-based pooling (Multiple Instance Learning):** Multiple cells per perturbation are pooled into one embedding with learned attention weights, which emphasize the cells with the strongest perturbation signal. 6.7× training speedup vs. instance-level processing.
...
- Strong zero-shot gene-gene relationship recovery on the RxRx3-core benchmark
...
**Relevance to MorphoCLIP:** Cross-well alignment handles batch effects well. It is a candidate regularizer alongside our contrastive objective.
...
Trained DINO, MAE, and SimCLR on a JUMP Cell Painting subset. DINO (ViT-S/16) beat CellProfiler in drug target and gene family classification, and generalized zero-shot to unseen genetic perturbation datasets without fine-tuning.
...
- **Richer text prompts:** CellCLIP uses simple templates. Adding pathway information, protein function, and DrugBank descriptions could improve the text encoder's biological understanding.
- **Parameter efficiency:** CellCLIP is 1.48B params while CWA-MSN gets better results with 22M. There is room for a parameter-efficient VLM approach.
- **Batch effect handling:** Most contrastive methods do not explicitly address batch effects. Combining cross-well alignment (CWA-MSN) with cross-modal learning (CellCLIP) is unexplored.
...
- **VLM reasoning:** No prior work uses the generative side of VLMs: describing in natural language what morphological changes a perturbation caused. This could enable interpretability and hypothesis generation.
