...
**Key efficiency strategy:** The DINOv3 backbone is frozen, so features are pre-extracted once and cached to disk. Extraction takes ~7 minutes per plate (~6 hours for all 51 plates) and produces ~3 GB of .pt files per plate. All later training runs on the cached features, needs only ~2 GB VRAM and ~3-5 minutes per epoch, and allows 50+ ablation experiments in a single day.
