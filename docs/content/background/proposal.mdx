...
<Callout type="info" emoji="💡">
  **In Plain English:** Drug discovery relies on testing thousands of chemicals and genetic changes
  on cells, then photographing them to see what happened. The problem is spotting that "this drug
  changed cells in the same way as turning off that gene". Current methods catch only 5-25% of
  known matches. MorphoCLIP reads both the cell images and text descriptions of the treatments,
  and learns which drugs and genes have similar effects. See the
...
Drug discovery depends on knowing a compound's mechanism of action (MoA): which gene or protein pathway it modulates. One strategy uses _morphological similarity_: if a chemical treatment and a genetic perturbation (e.g., a CRISPR knockout) produce visually similar changes in cells, the drug likely targets that gene's pathway. The CPJUMP1 benchmark (Chandrasekaran et al., 2024) frames this as a retrieval task: given a compound's Cell Painting image, retrieve its known gene target from a gallery of genetic perturbation images, ranked by embedding similarity.

Current methods do not do well here. Handcrafted CellProfiler features with cosine similarity reach only 5-25% fraction matching. Deep learning methods improve on this but share three problems: batch effects confound the learned representations, compound and gene perturbations live in separate embedding spaces, and most methods train on compounds only, leaving genetic perturbations to evaluation time.

## 2. Proposed Approach

We propose **MorphoCLIP**, a contrastive learning framework that uses natural-language descriptions of perturbations as a supervision signal during training. The core hypothesis is that text supervision _complements_ batch correction and gene-inclusive training: it adds semantic structure that purely visual methods miss. Prior work supports the combination. CellCLIP (Lu et al., 2025) has text supervision but no batch correction and no gene training data, and performs below the CellProfiler baseline on cross-class mAP. CWA-MSN (Huang et al., 2025) has batch correction and gene training but no text, and outperforms CellCLIP. No single ingredient suffices alone. MorphoCLIP's contribution is the three-way combination: text supervision, cross-well batch correction, and gene-inclusive training.

**How it works at training time:** Each Cell Painting image is paired with a text description of its perturbation, either a compound description or a gene description (see Text Prompt Engineering below). A contrastive loss aligns images with their matching text descriptions in a shared 512-dimensional space. Both compound and gene perturbations are encoded as text during training, so the model sees the full perturbation landscape rather than compounds alone.

**How it works at evaluation:** For the standard CPJUMP1 benchmark, we follow the established protocol: image-to-image retrieval using the learned embeddings. Compound images are compared against gene perturbation images by cosine similarity. The text encoder is not needed at inference; its role is to shape the embedding space during training. We also report text-only retrieval (compound image to gene text) as a secondary analysis, to check that the learned space is semantically structured.

**Architecture:**

**Image Encoder.** A frozen DINOv3 ViT-L/16 (~300M params, 1024-dim CLS token; Simeoni et al., 2025) backbone extracts per-channel CLS tokens from each of the 5 fluorescence channels (DNA, ER, RNA, actin/Golgi/plasma membrane, mitochondria) at 384x384 resolution. DINOv3 is Meta AI's latest self-supervised vision model; gram anchoring and improved training recipes give it better features than DINOv2. A small CrossChannelFormer (2-layer transformer) aggregates the per-channel tokens into a single image representation, followed by a projection head to 512 dimensions. CellCLIP uses DINOv2-g (1.1B params), a ~4x larger backbone. MorphoCLIP defaults to DINOv3-L, which has strong features and is cheap to pre-extract (~7 minutes per plate on an RTX 5080). LoRA adapters (rank 8-16) are an ablation variant, not part of the default method; the default pipeline uses pre-extracted features from a fully frozen backbone.

**Text Encoder.** A frozen BioClinical ModernBERT (Sounack et al., 2025; 150M params, 768d output) with a trainable projection head encodes perturbation descriptions into the shared latent space. BioClinical ModernBERT is the current state-of-the-art biomedical encoder, built on ModernBERT with RoPE, Flash Attention, and GeGLU activations, and pre-trained on 53.5 billion tokens of PubMed, PMC, and clinical text. That is a large step up from the PubMedBERT (2020, 110M, 3.1B tokens) used by CellCLIP. Its pre-training corpus contains extensive molecular nomenclature, gene names, and pathway descriptions. Unlike SMILES-specialized models (e.g., ChemBERTa, SMI-TED), which cannot encode gene descriptions, it handles both compound and gene text in one model. Whether it can meaningfully encode SMILES strings (chemical notation, not natural language) is uncertain; the multi-scale text ablation tests this explicitly (see Section 5) and it is flagged as a risk (see Section 6).

**Training Objective.** Continuously Weighted Contrastive Loss (CWCL; Huang et al., 2025) handles inactive perturbations and many-to-one mappings. Cross-well alignment (CWA; Huang et al., 2025) regularizes representations against batch effects during training. That matters because batch effects are the dominant confound in Cell Painting data.

**Text Prompt Engineering.** The quality and format of the text descriptions strongly affect model performance. We define structured templates for each perturbation type, drawing on external biological databases:
...
**Negative Mining Strategy.** Contrastive learning is sensitive to how negative pairs are built. In perturbation matching, two compounds targeting the same gene should be soft positives (high similarity), not hard negatives; a standard contrastive loss would wrongly push them apart. CWCL (Huang et al., 2025) fixes this by replacing binary positive/negative labels with continuously weighted similarity scores, which keeps the many-to-one structure where several compounds map to one gene target. Cross-well alignment additionally pulls replicates of the same perturbation together across wells and batches, so batch effects do not fragment the embedding space.
...
The key observation from this table: _no existing method combines text supervision with gene training data and batch correction._ CellCLIP has text but no genes and no batch correction. CWA-MSN has genes and batch correction but no text. MorphoCLIP combines all three. CellCLIP falling below the CellProfiler baseline suggests text supervision alone is not enough without batch correction, and may even hurt when batch effects dominate the embedding space. That motivates the combined approach: CWA neutralizes batch effects so text supervision can contribute its semantic structure.
...
- **Factorial ablation study.** The most important experiment disentangles the contribution of each component. We use a factorial design over three binary factors: text supervision, CWA batch correction, and gene training data.
  ...
  This design directly tests whether text supervision contributes beyond batch correction alone. If removing text (row 2) matches full performance, the paper pivots to documenting when text helps and when it does not, itself a useful finding.
...
- **Few-shot and zero-shot evaluation.** We hold out a subset of compounds and genes during training. At inference, we test whether held-out compounds can be matched to their gene targets by text-only retrieval (compound image to gene text description). Purely visual methods cannot do this at all; it is an advantage unique to text-supervised approaches.

- **Batch effect visualization:** UMAP of embeddings colored by plate/batch before and after CWA, to show whether the correction works.
...
**Key efficiency strategy:** The DINOv3 backbone is frozen, so features are pre-extracted once and cached to disk. Extraction takes ~7 minutes per plate (~6 hours for all 51 plates) and produces ~75 MB of .pt files per plate. All later training runs on the cached features, needs only a few GB of VRAM and, with the cache preloaded into RAM, about 4 seconds per epoch, which allows dozens of ablation runs in a single day.

| Configuration                                  | Trainable Params | Est. VRAM | Time/Epoch |
| ---------------------------------------------- | ---------------- | --------- | ---------- |
| Pre-extracted DINOv3-L + CrossChFormer + heads | ~14M             | ~2 GB     | ~4 s (cache in RAM) |
