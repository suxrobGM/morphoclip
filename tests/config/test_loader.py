...
written twice (once per training package). Testing the loader directly is what
lets the two packages stop carrying their own copy.
"""
...
class TestMergeDicts:
    ...
    def test_a_non_mapping_value_replaces_wholesale(self) -> None:
...
class TestResolveExtends:
    def test_grandparents_resolve_through_the_chain(self, tmp_path: Path) -> None:
        """Also pins that the child wins, and that `extends` is stripped."""
...
class TestDottedOverrides:
    ...
    @pytest.mark.parametrize("item", ["dataset.batch_size", "batch_size=8", ".batch_size=8"])
    ...
    def test_overrides_apply_in_order(self) -> None:
        assert apply_overrides({}, ["s.size=1", "s.size=2"]) == {"s": {"size": 2}}
...
class TestLoadConfig:
    ...
    def test_a_dotless_exponent_still_reaches_the_model_as_a_float(self, tmp_path: Path) -> None:
        """A YAML 1.1 quirk: `1e-4` parses as a str, so the model has to coerce it."""
        child = write(tmp_path / "child.yaml", {})
        assert load_config(Root, child, ["section.rate=1e-4"]).section.rate == 1e-4


def test_assigning_a_wrong_type_raises_rather_than_sticking() -> None:
    section = Section()
